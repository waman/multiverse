...
class Energy[A: Fractional](val value: A, val unit: EnergyUnit)
  ...
  def toAbsoluteTemperature: AbsoluteTemperature[A] = new AbsoluteTemperature(
      apply(EnergyUnitObjects.joule) * implicitly[Fractional[A]].fromReal(1 / Constants.BoltzmannConstant),
      AbsoluteTemperatureUnitObjects.kelvin)


  def toMass: Mass[A] = new Mass(
      apply(EnergyUnitObjects.joule) * implicitly[Fractional[A]].fromReal(1 / (Constants.SpeedOfLight * Constants.SpeedOfLight)),
