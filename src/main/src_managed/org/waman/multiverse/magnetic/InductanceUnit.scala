...
object InductanceUnit extends ConstantsDefined[InductanceUnit]{
  ...
  case object MicroHenry extends IntrinsicInductanceUnit("MicroHenry", Seq("μH", "mcH"), r"1e-6")
...
trait InductancePostfixOps[A]{
  ...
  def mcH : A = inductancePostfixOps(MicroHenry)
...
trait InductanceDot[A]{
  ...
  def mcH(dot: Dot): A = inductanceDot(MicroHenry)
...
trait InductancePer[A]{
  ...
  def mcH(per: Per): A = inductancePer(MicroHenry)
