...
object SolidAngleUnit extends ConstantsDefined[SolidAngleUnit]{
  ...
  case object MicroSteradian extends IntrinsicSolidAngleUnit("MicroSteradian", Seq("μsr", "mcsr"), r"1e-6")
...
trait SolidAnglePostfixOps[A]{
  ...
  def mcsr : A = solidAnglePostfixOps(MicroSteradian)
...
trait SolidAngleDot[A]{
  ...
  def mcsr(dot: Dot): A = solidAngleDot(MicroSteradian)
...
trait SolidAnglePer[A]{
  ...
  def mcsr(per: Per): A = solidAnglePer(MicroSteradian)
