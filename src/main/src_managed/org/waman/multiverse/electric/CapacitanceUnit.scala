...
object CapacitanceUnit extends ConstantsDefined[CapacitanceUnit]{
  ...
  case object MicroFarad extends IntrinsicCapacitanceUnit("MicroFarad", Seq("μF", "mcF"), r"1e-6")
...
trait CapacitancePostfixOps[A]{
  ...
  def mcF : A = capacitancePostfixOps(MicroFarad)
...
trait CapacitanceDot[A]{
  ...
  def mcF(dot: Dot): A = capacitanceDot(MicroFarad)
...
trait CapacitancePer[A]{
  ...
  def mcF(per: Per): A = capacitancePer(MicroFarad)
