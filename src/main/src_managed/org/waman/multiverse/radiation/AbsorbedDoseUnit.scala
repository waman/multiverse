...
object AbsorbedDoseUnit extends ConstantsDefined[AbsorbedDoseUnit]{
  ...
  case object MicroGray extends IntrinsicAbsorbedDoseUnit("MicroGray", Seq("μGy", "mcGy"), r"1e-6")
...
trait AbsorbedDosePostfixOps[A]{
  ...
  def mcGy : A = absorbedDosePostfixOps(MicroGray)
...
trait AbsorbedDoseDot[A]{
  ...
  def mcGy(dot: Dot): A = absorbedDoseDot(MicroGray)
...
trait AbsorbedDosePer[A]{
  ...
  def mcGy(per: Per): A = absorbedDosePer(MicroGray)
