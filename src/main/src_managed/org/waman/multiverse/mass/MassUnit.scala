...
object MassUnit extends ConstantsDefined[MassUnit]{
  ...
  case object MicroGram extends IntrinsicMassUnit("MicroGram", Seq("μg", "mcg"), r"1e-6" * r"1e-3")
  ...
  case object Gamma extends IntrinsicMassUnit("Gamma", Seq("γ"), 1, MicroGram)
...
trait MassPostfixOps[A]{
  ...
  def mcg : A = massPostfixOps(MicroGram)
...
trait MassDot[A]{
  ...
  def mcg(dot: Dot): A = massDot(MicroGram)
...
trait MassPer[A]{
  ...
  def mcg(per: Per): A = massPer(MicroGram)
