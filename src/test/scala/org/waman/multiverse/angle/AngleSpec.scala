  ...
  "3.0 <<angle unit>> should be converted to the equivalent value in radian" in {
    ...
    val conversions =
      Table(
        ...
        (Seq(3.0.microRadian, 3.0 microRadian, 3.0 (microRadian)), 3e-6),
        (Seq(3.0.microRad, 3.0 microRad, 3.0 (microRad)), 3e-6),
  ...
  "3.0 rad should be converted to the equivalent value in other angle units" in {
    ...
    val conversions =
      Table(
        ...
        (Seq(q.microRadian, q microRadian, q (microRadian)), 3e6),
        (Seq(q.microRad, q microRad, q (microRad)), 3e6),
