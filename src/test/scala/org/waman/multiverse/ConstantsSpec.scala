...
class ConstantsSpec extends MultiverseCustomSpec {
  ...
  "R∞ (rydberg constant) ~ m_e e^4 / (8ε_0^2 h^3 c)" in {
    val sut = RydbergConstant.toDouble
  ...
  "k (Coulomb constant) ~ 1 / (4πε_0)" in {
    val sut = CoulombConstant.toDouble
  ...
  "Physical constants in CGS" in {
    ...
    import org.waman.multiverse.unit.defs.MassUnits.u
    ...
    import org.waman.multiverse.unit.custom.ElectromagneticUnits._
    // Exercise
    val conversions =
      Table(
        ("current", "expected"),
